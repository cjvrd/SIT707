...
public class RoutingServlet {
	...
	public RedirectView q1(
		...
		Double resultUser;
		try {
			resultUser = Double.parseDouble(request.getParameter("result"));
		} catch (Exception e) {
			resultUser = null;
		}
		...
		if (calculatedResult == null || resultUser == null) {
			//checking for invalid input, calculated result is handled in MathQuestionService
			redirectView = new RedirectView("/q1", true);
			redirectAttributes.addFlashAttribute("message", "Please enter a valid input.");

		} else if (calculatedResult.equals(resultUser)) {
	...
	public RedirectView q3(HttpServletRequest request, RedirectAttributes redirectAttributes) {
		...
		if (calculatedResult == null || resultUser == null) {
			...
			redirectView = new RedirectView("/q3", true);
			redirectAttributes.addFlashAttribute("message", "Congratulations, you have completed the test!");
